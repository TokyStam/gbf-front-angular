...
export class CreatBudgetComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sectionService.fetchComtpeBySection(
      this.sectionService.filterCompte(60),
      this.compte60
    );
    this.sectionService.fetchComtpeBySection(
      this.sectionService.filterCompte(61),
      this.compte61
    );
    this.sectionService.fetchComtpeBySection(
      this.sectionService.filterCompte(62),
      this.compte62
    );
    this.sectionService.fetchComtpeBySection(
      this.sectionService.filterCompte(65),
      this.compte65
    );
  }
  initGroup(nomGroup = "all") {
    ...
    if (nomGroup === "all") {
      ...
      let achatServiceChargePerma = this.budgetForm.get(
        "achatServiceChargePerma"
      ) as FormArray;
  ...
  onSubmitForm() {
    ...
    this.budgetForm.value.achatDeBiens.map(elem => {
      const budget = {
        montant: elem.montant,
        annee: year,
        compteId: elem.compte,
  ...
  private getComptes(id) {
    this.artcileService.getComptes(id).subscribe(
      (data: any) => {
        this.compte60 = [...data];
      },
      error => {
